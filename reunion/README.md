...
1. 管理画面 `/admin/settings/reunion` で同窓会情報を入力（名称・日時・会場・会費・振込先など）
2. `/admin/settings/mail` でメール送信設定を行う（Brevo API キーなど）
3. `/admin/roster` から名簿 CSV をインポートする

### 参加管理の流れ

**1. 仮出欠の収集**

- 参加者に `/form/provisional` の URL を共有する
- 参加者が氏名・メールアドレス・参加意思を回答すると確認メールが届く
- 管理画面の参加者一覧で回答状況を確認できる

**2. 本出欠 URL の送信**

- `/admin/mail-hub` の「本出欠URL送信」で仮出欠回答済みの全員に一括送信
- 送信済みの人は自動スキップされるため、繰り返し押しても重複しない
- 個別送信は参加者詳細画面のボタンから行う

**3. 本出欠・振込の確認**

- 参加者がトークン付き URL から本出欠を回答・振込名義を入力
- 銀行から出力した CSV を `/admin/payments` でアップロードすると自動照合される
- 照合できなかったものは手動で紐付ける

**4. リマインド送信**

- `/admin/mail-hub` からリマインド・最終リマインドを一括送信できる
- 最終リマインドには案内 PDF を添付可能

**5. 直前キャンセルの受付**

- 本出欠締切後も参加者が自分のフォームから「キャンセル」できる（`final_reminder_deadline` まで）
